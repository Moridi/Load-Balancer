...
public:
	...
	inline std::vector<std::string> tokenize(const std::string& line, const char delimiter)
			const noexcept;
	...
	inline void set_argv(const std::vector<std::string>& files_name, char*** argv);
	inline void set_file_arguments(char*** argv, int& index,
			const std::vector<std::string>& files_name);
	inline void set_argv_element(char*** argv, int index, std::string value);
	...
	void read_from_pipe(int begin, int end, std::vector<std::string>& files_name,
			int file_descriptor[]);
	void write_to_pipe(int begin, int end, int file_descriptor[]);
	void exec_worker(const std::vector<std::string>& files_name, int file_descriptor[]);
	void fill_dataset(DIR* directory);

private:
	static constexpr int BEGIN = 0;
	static constexpr int READ_DESCRIPTOR = 0;
